...
def get_embedding_for_assembly(assembly, tokenizer, model):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = model.to(device)
    # Tokenize the assembly code using RobertaTokenizer
    tokens = tokenizer.encode(assembly, truncation=True, max_length=103, padding='max_length')
    ...
    # Extract the pooler output (representing the entire sequence)
    # This is usually more suitable for downstream tasks
    embeddings_tensor = outputs.pooler_output

    # Convert tensor to numpy array
    embedding_array = embeddings_tensor.detach().cpu().numpy()
...
def train_node_classifier_batch(clf_node, node_features_batch, node_labels_batch, batch_index):
    if len(np.unique(node_labels_batch)) < 2:
        print(f"Skipping batch {batch_index} due to insufficient class diversity.")
        return None  # Skip this batch

    node_features_batch = np.array(node_features_batch)
    node_labels_batch = np.array(node_labels_batch)
    clf_node.fit(node_features_batch, node_labels_batch)
    with open(f'./saved_kernel/node_model_batch_{batch_index}.pkl', 'wb') as f:
        pickle.dump(clf_node, f)
    print(f"Training node batch {batch_index} - Features shape: {node_features_batch.shape}, Labels shape: {node_labels_batch.shape}")
    return clf_node
def train_graph_classifier(train_df, val_df, test_df, le):
    ...
    val_cfg, val_node_labels, val_graph_labels, val_vul_nodes_count, val_non_vul_nodes_count, val_total_nodes_count = create_cfg_for_graph(
        val_df)
    print("Number node labels in validating data:", len(val_node_labels))
    print("Number cfg in validating data:", len(val_cfg))
    print("Number of vulnerable nodes in validating data:", val_vul_nodes_count)
    print("Number of non-vulnerable nodes in validating data:", val_non_vul_nodes_count)
    print("Number of total nodes in validating data:", val_total_nodes_count)
    test_cfg, test_node_labels, test_graph_labels, test_vul_nodes_count, test_non_vul_nodes_count, test_total_nodes_count = create_cfg_for_graph(
        test_df)
    ...
    print("Unique node labels in training data:", set([label for _, label in train_node_labels]))
    print("Count of each node label in training data:",
          {label: sum(1 for _, l in train_node_labels if l == label) for label in
           set([label for _, label in train_node_labels])})


    batch_size = 1000
    gk = GraphKernel(kernel={"name": "weisfeiler_lehman", "n_iter": 5}, normalize=False, n_jobs=-1)
    # gk = GraphKernel(kernel={"name": "subtree_wl"}, normalize=False, n_jobs=-1)
    # //gk = GraphKernel(kernel={"name": "shortest_path"}, normalize=False, n_jobs=-1)
    # gk = GraphKernel(kernel={"name": "pyramid_match"}, normalize=False, n_jobs=-1)  # Pyramid match kernel
    # gk = GraphKernel(kernel={"name": "graphlet_sampling"}, normalize=False, n_jobs=-1)  # Graphlet sampling kernel
    # gk = GraphKernel(kernel={"name": "propagation"}, normalize=False, n_jobs=-1)  # Propagation kernel
    # gk = GraphKernel(kernel={"name": "neighborhood_hash"}, normalize=False, n_jobs=-1)  # Neighborhood hash kernel


    svc = SVC(kernel="precomputed", probability=True, random_state=random_state)  # Enable probability estimates
    ...
    for i, (train_batch, train_labels_batch) in enumerate(
        ...
        train_labels_batch = np.array(train_labels_batch).flatten()  # Ensure train_labels_batch is 1D
...
def tran_roberta_model(model, train_loader,device):
...
def evaluate_roberta_model(model, dataloader, device):
...
def test_roberta_model(model, dataloader, device):
    ...
    avg_test_accuracy = total_eval_accuracy / len(dataloader.dataset)
    avg_test_loss = total_eval_loss / len(dataloader)
...
def train_node_classifier(train_df, val_df, test_df, le):
    ...
    train_loader = DataLoader(train_dataset, batch_size=10097152, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=10097152, shuffle=False)
    test_loader = DataLoader(test_dataset, batch_size=10097152, shuffle=False)

    input_dim = train_node_features.shape[1]
    model = MLP(input_dim).to(device)  # Chuyển mô hình sang GPU
    ...
    print(classification_report(test_true_labels, test_predictions, digits=5))
...
def train_all():
   ...
   # Fit LabelEncoder với toàn bộ opcode từ tất cả các tập dữ liệu
    ...
    return assembly_length, clf_node
